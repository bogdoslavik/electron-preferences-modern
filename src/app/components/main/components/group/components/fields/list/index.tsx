...
class ListField extends React.Component {
    ...
    render() {
        return (
            <div className={`field field-list key-${this.field.key}`}>
                <div className="field-label" aria-label={this.label}>
                    {this.label}
                </div>
                <div>
                    <div>
                    ...
                    <div className="ep-list-button-container">
                        <button
                            className="ep-list-button"
                            onClick={this.addClick}
                            aria-label="Add"
                            disabled={this.max > 0 && this.length >= this.max}
                        >
                            <span className="ep-list-button-text">+</span>
                        </button>
                        <button
                            className="ep-list-button"
                        ...
                        {this.orderable && (
                            <React.Fragment>
                                <button
                                    ...
                                    aria-label="Move up"
                                >
                                    <span className="ep-list-button-text">
                                        ↑
                                    </span>
                                </button>
                                <button
                    ...
                    >
                        <div className="ep-list-modal-container">
                            <div className="ep-list-modal-input-container">
                                <label className="ep-list-modal-input-label">
                                    {this.addItemLabel}
                                </label>
                                <input
                                    className="ep-list-modal-input"
                                    type="text"
                                    value={this.state.itemToAdd}
                                    autoFocus={true}
                                    onChange={this.itemToAddChanged}
    ...
    removeClick() {
        if (this.state.selectedIndex >= 0) {
            this.props.onChange(
                this.value.filter(
                    (item, index) => index !== this.state.selectedIndex,
                ),
            );
        }
    }
